...
class Any6D:
    ...
    # guess_translation_bounding_box and guess_translation estimate the initial pose.
    ...
    # This function can be used to compute the anchor pose (or directly the solution?)
    def register_any6d(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5, name=None, refinement=True, axis_align=True,coarse_est=True):
        ...
        if coarse_est:
            ...
            # Once the bb are aligned, we look for the best combination of axes' scaling to match the observed point cloud and scale the mesh accordingly
        ...
        poses = poses.data.cpu().numpy()
        # Finding the center based on the points inside the mask
        center = self.guess_translation(depth=depth, mask=ob_mask, K=K)

        poses = torch.as_tensor(poses, device='cuda', dtype=torch.float)
        # Adding the same translation to all the generated poses (only rotation is different)
        poses[:, :3, 3] = torch.as_tensor(center.reshape(1, 3), device='cuda')
        ...
        if refinement:
            ...
            # Because the depth (xyz_map) coordinates are observed from the camera, we need to put them in the object coordinates (mesh system)
            points = xyz_map[ob_mask].reshape(-1, 3)
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(points)
