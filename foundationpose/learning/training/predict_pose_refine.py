...
class PoseRefinePredictor:
  def __init__(self, ckpt_root_dir='/home/../Experiments/simonep01/checkpoints/FP_weights'):
  ...
  def predict(self, rgb, depth, K, ob_in_cams, xyz_map, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, iteration=5):
    '''
    @rgb: np array (H,W,3)
    @ob_in_cams: np array (N,4,4)  # Candidate poses
    '''
    ...
    for _ in range(iteration):
      ...
      # This function will render the object at ob_in_cams, and crop the observed rgb, depth, normal, xyz_map according to the bounding box of the rendered object
      ...
      for b in range(0, pose_data.rgbAs.shape[0], bs):
        ...
        with torch.amp.autocast(device_type="cuda", enabled=self.amp):
          output = self.model(A,B) # Gives trans_delta and rot_mat_delta
        for k in output:
